...
module TestReportKit
  class Runner
    ...
    def simplecov_init_content
      # Pre-register every project source file so the denominator stays stable across
      # single-process, sharded, and parallel-merged runs. Without this, Zeitwerk's
      # lazy loading makes "files seen" depend on which specs ran in which process.
      track_files_line =
        if @config.simplecov_track_files
          "track_files #{@config.simplecov_track_files.inspect}"
        else
          ""
        end
